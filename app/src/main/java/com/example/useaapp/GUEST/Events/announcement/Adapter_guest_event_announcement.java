...
public class Adapter_guest_event_announcement extends RecyclerView.Adapter<Adapter_guest_event_announcement.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.Title_guest_event_item.setText(data.get(position).getAnnounce_title());
        holder.Detail_guest_event_item.setText(data.get(position).getAnnounce_detail());
        holder.Day_guest_event_item.setText(data.get(position).getAnnounce_day());
        holder.Month_guest_event_item.setText(data.get(position).getAnnounce_month());
        ...
        Glide.with(holder.Title_guest_event_item.getContext()).load("http://172.17.18.76/USEA/Guest/event_image/" + data.get(position).getAnnounce_image()).into(holder.eventImg);
    }
